...
export function mergeMessages(existing: Message[], incoming: Message[]): Message[] {
  ...
  for (const m of incoming) {
    ...
    if (i < 0) list.push(m);
    // An HTTP failure or a stale optimistic retry can arrive after the canonical event.
...
export const useWorkspace = create<State>(set => ({ ...initial,
  applyBootstrap: (b, accountKey) => set(s => {
    ...
    const messages = sameAccount ? Object.fromEntries(Object.entries(s.messages).filter(([id]) => id in conversations)) : {};
    const drafts = sameAccount ? Object.fromEntries(Object.entries(s.drafts).filter(([id]) => id in conversations)) : {};
    return { bootstrap:b, accountKey, conversations, messages, drafts, files:b.permissions.canDownload ? b.files : [], ready:true, error:'', cursor:b.eventCursor };
  }),
