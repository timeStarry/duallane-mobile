...
export const bootstrapSchema = z.object({ auth: z.object({ currentUser: memberSchema }), space: z.object({ id, name: z.string() }), eventCursor: z.number().int().nonnegative(), permissions: z.object({ canCreateDirect: z.boolean().default(false), canCreateGroup: z.boolean().default(false), canUpload: z.boolean().default(false), canDownload: z.boolean().default(false) }).default({ canCreateDirect: false, canCreateGroup: false, canUpload: false, canDownload: false }), policy: z.object({ dailyQuotaBytes: z.number(), remainingQuotaBytes: z.number(), messageRetentionCount: z.number() }), members: z.array(memberSchema), conversations: z.array(conversationSchema), files: z.array(attachmentSchema) });
...
export function parseMessage(input: unknown): Message | null {
  ...
  return { ...base, blocks: !fallback && content.success ? content.data.blocks : [], attachments: fallback ? [] : base.attachments, plainText: base.plainText || (content.success ? content.data.plainText ?? '' : '') || (fallback ? '此消息暂不支持，请更新应用后查看' : ''), fallback };
}
