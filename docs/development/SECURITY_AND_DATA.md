...
- GitHub 使用 PKCE + 系统 Custom Tabs；不使用 WebView 收集密码。
...
日志只记录匿名诊断、错误 code、route 族、HTTP status、耗时和版本；不记录 token、消息正文、文件名、
邀请链接、URL query、对象 key、完整 URL 或实时 seq。传输失败会把诊断 code 附在用户可见文案后，
便于对照 logcat。本地通知默认只显示“有新消息”，遵守会话
